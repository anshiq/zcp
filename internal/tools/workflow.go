...
type WorkflowInput struct {
	...
	Workflow string `json:"workflow,omitempty" jsonschema:"Workflow name: bootstrap, develop, or export. For recipe authoring use the dedicated zerops_recipe tool."`
	...
	Region                string   `json:"region,omitempty"                jsonschema:"Launch-production only: target region code (default 'eu-central'). Other values from Zerops dashboard."`
	CustomDomain          string   `json:"customDomain,omitempty"          jsonschema:"Launch-production only: optional custom domain. If set, ZCP synthesizes DNS records + verification probes; user attaches the domain in Zerops UI (no L7 mutation by ZCP in v1)."`
	...
	// LaunchKey is the one-shot account-wide Zerops API token used during
	// the mutation window. Generated by the user in Zerops dashboard
	// (Settings → Access Tokens Management), passed on the publish call
	// only, and discarded after launched/failed status. P-LP-1: never
...
func handleWorkflowAction(ctx context.Context, projectID string, engine *workflow.Engine, client platform.Client, httpClient ops.HTTPDoer, cache *ops.StackTypeCache, schemaCache *schema.Cache, logFetcher platform.LogFetcher, input WorkflowInput, stateDir, selfHostname string, mounter ops.Mounter, sshDeployer ops.SSHDeployer, rt runtime.Info) (*mcp.CallToolResult, any, error) {
	...
	case "start":
		...
		if input.Workflow == workflowLaunchProduction {
			return handleLaunchProduction(ctx, projectID, client, input, stateDir, rt)
		}
