...
the body must give the porter a path forward — finish teaching the
mechanism in-body, OR add a markdown link whose visible link text is
a porter-readable descriptive label (not an internal corpus slug ID).

Naming the guide by topic without a URL ("the X guide on Zerops docs
covers Y") leaves the porter with a pointer they can't follow.
**Equally bad: using the internal `zerops_knowledge` corpus slug
(`init-commands`, `env-var-model`, `managed-services-nats`,
`rolling-deploys`, `cross-service-refs`, `object-storage`,
`http-support`) as the link text** — the slug ID isn't
porter-recognizable; it leaks the recipe-engine's lookup vocabulary
into porter prose. The goldens consistently use descriptive labels
(`[Laravel documentation](https://laravel.com/docs/...)`,
`[step-by-step tutorial](https://docs.zerops.io/...)`,
`[zsc health-check](https://docs.zerops.io/...)`) — never a corpus
slug as link text.
...
Same migrate/seed body, ending with `[init-commands](url)` link
text. Real URL — but the visible label is the internal
`zerops_knowledge` corpus slug ID. Porters never interact with that
corpus; the slug isn't porter-readable. Forbidden middle ground
with a URL bolted on — score 7.0, not 8.5. Run-32 carried this
shape because URL presence was mistaken for resolution; URL
presence is necessary but not sufficient.

### 8.5 anchor — in-body completion (preferred for Zerops mechanics)
...
Body teaches the mechanism (per-deploy ledger, in-script-guard
semantics) directly. Porter has no need to leave the page.

### 8.5 anchor — descriptive-labeled link variant

Same migrate/seed body as the 7.0 variant, ending with:
`The [per-deploy `+"`"+`initCommands`+"`"+` reference](https://docs.zerops.io/zerops-yaml/specification#initcommands-)
covers key shape and the in-script-guard pitfall.`

Same URL as the 7.0 variant; the link text is the difference. The
descriptive label names the porter-recognizable concept; the corpus
slug (`init-commands`) doesn't appear on the page. Acceptable when
the body would otherwise sprawl; in-body completion remains
preferred for Zerops mechanics.

### 9.0 anchor — application-specific corollary (in-body OR descriptive link)

8.5 in-body or descriptive-link variant + one sentence drawing the
line between the platform's general teaching and this recipe's
application — e.g. *"the corollary here is that splitting the key
across migrator and seeder lets you re-fire the seed independently
when its dataset changes — without re-applying migrations that have
already settled."* The corollary is load-bearing — the porter
learns *why this recipe* makes the choice the platform describes.
...
- **Real markdown link with descriptive label** — body contains
  `[descriptive label](https://...)` pointing at the relevant docs URL.
  The label MUST name the SUBJECT (e.g. `[Laravel documentation]`,
  `[zsc health-check]`), NOT the corpus slug verbalized as English
  (`[managed NATS service]`, `[Zerops env-var model]` — these are
  internal taxonomy spelled out and count as no resolution per the
  spec's slug-name-link-text ban).
...
- **Topic-name handwave** — body names the guide by topic ("the X
  ...
  resolution.
- **Slug-name as link text** — body uses a corpus slug ID or its
  English-cased translation as link label (`[init-commands](url)`,
  `[managed NATS service](url)`). Treat this as no resolution; the
  link form is forbidden middle ground per spec §"Citation prose."
- **No citation** — topic on map, body silent on the platform
  mechanism.

| Topic-on-map matches with real-link OR in-body-complete | Score |
|---|---|
| 0% | 7.0 |
| 1–49% | 7.5 |
| 50–99% | 8.5 |
...
Topic-name handwaves count as `0%` — they're the forbidden middle
ground. The "application-specific corollary" pattern: *"The X
...
The link text is *"Zerops managed NATS service"* — descriptive prose,
not the corpus slug `managed-services-nats`. A
`[managed-services-nats](url)` link with the same URL would score
7.0, not 8.5: same URL, but the visible text leaks the corpus slug.
