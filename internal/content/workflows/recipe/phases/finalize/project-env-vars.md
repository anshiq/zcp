...
Dual-runtime recipes declare cross-service URL constants at project level so every runtime service in that env sees the same `DEV_API_URL` / `API_URL` / `DEV_FRONTEND_URL` / `FRONTEND_URL`. Single-runtime recipes without cross-service URL constants omit `projectEnvVariables` entirely — the template renders the shared secret on its own.
