...
## 2026-05-07 — Run-31 Fix #4 closure + F-45 (screenshot close-gate + tier-intro closure-of-expectation)

Closes two open items from `runs/31/ANALYSIS.md` §"F-track verdict update".

**Fix #4 (HIGH) — engine close-gate for feature-pass screenshot capture.** Run-31 evidence: `runs/31/screenshots/` did not exist; all 6 `browser_verification` facts in `runs/31/environments/facts.jsonl` carried `"screenshot":"none-snapshot-only"`; the features-frontend agent issued only `snapshot -i -c` calls. Brief teaching at `briefs/feature/integration_validator.md` read as soft instruction ("Capture", "Add it", "Substitute") with no MUST / required-fact gate; the brief-string pin (`TestIntegrationValidatorAtom_TeachesScreenshotAtClose`) only asserted text presence. New gate `gateFeatureScreenshotCaptured` (`internal/recipe/gate_feature_screenshot.go`) refuses `complete-phase phase=feature` when no `browser_verification` fact in the run carries `extra.screenshot` set to a path under `<outputRoot>/screenshots/`. Positive presence-check (system.md §4 catalog-drift-safe shape — not a content ban-list / regex over brief text). Wired via new `FeatureGates()` helper (`internal/recipe/gates.go`), feature-only — scaffold has no browser-walk close-step. Brief edit at `integration_validator.md` adds imperative-mandate framing (MUST capture, gated-by `complete-phase phase=feature`), worked example of the close-step `zerops_browser` batch with `screenshot --full` as LAST command, worked example of the matching `browser_verification` fact carrying `extra.screenshot` (NOT `none-snapshot-only`). Sim driver (`cmd/zcp-recipe-sim/gate.go`) gains a `feature` entry mirroring prod; parity test `TestGatesForPhase_SimProdParity` updated. Pinned by `TestGateFeatureScreenshot_*` (5 tests) + `TestFeatureClose_RefusesWithoutScreenshotFact` (end-to-end) + extended `TestIntegrationValidatorAtom_TeachesScreenshotAtClose`.

**F-45 (LOW) — closure-of-expectation guard for tier README intros.** Run-31 evidence: `runs/31/environments/5 — Highly-available Production/README.md:7` carried "Meilisearch and object-storage stay single-shape — no HA option in those families" — same closure-of-expectation shape F-33-broader forbids in service-block yaml comments, on a different surface (tier README intro) authored from a different step in the same env-content brief (`per_tier_authoring.md` workflow §2 first bullet). Atom edit at `briefs/env-content/per_tier_authoring.md` adds a new section "Tier README intros — closure-of-expectation guard" with BAD/GOOD worked example (BAD: the literal run-31 tier 5 intro phrasing; GOOD: rephrase to within-tier deployment-shape vocabulary "single-instance Meilisearch and object-storage with the 1 GB managed-RAM floor"). Pinned by `TestEnvContentIntroAuthoringAtom_TeachesClosureOfExpectationGuard` (`atoms_intro_authoring_run31_test.go`).
